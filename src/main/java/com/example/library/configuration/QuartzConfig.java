...
public class QuartzConfig {
    ...
    public JobDetail sendEmailJobDetails() {
        return JobBuilder.newJob(SendEmailNotificationJob.class)
                         .withIdentity(SendEmailNotificationJob.class.getSimpleName() + " identity").storeDurably()
                         .build();
    }

    @Bean
    public Trigger sendEmailJobTrigger(JobDetail sendEmailJobDetails) {
        return TriggerBuilder.newTrigger().forJob(sendEmailJobDetails)
                             .withIdentity(SendEmailNotificationJob.class.getSimpleName() + " trigger").withSchedule(
